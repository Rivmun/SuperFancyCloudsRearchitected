...
public class CloudData implements CloudDataImplement {
	...
	private static SimplexNoise cloudNoise;
	...
	public void tick() {
		lifeTime -= Minecraft.getInstance().getTimer().getGameTimeDeltaTicks() * 0.25f * 0.25f;
	}
				...
				for (int cz = 0; cz < width; cz++) {
					...
					if (CONFIG.isEnableWeatherDensity() && CONFIG.isBiomeDensityByChunk()) {
						if (CONFIG.isBiomeDensityUseLoadedChunk()) {
							var vec = new Vec2(cx - width / 2, cz - width / 2).normalized();		// stepping pos near towards to player
							var px2 = px;
							var pz2 = pz;
							while (!world.getChunkSource().hasChunk((int) px2 / 16, (int) pz2 / 16) && Math.abs(scrollX - px) + Math.abs(scrollZ - pz) > CONFIG.getCloudBlockSize() * 4) {
							...
							f = !CONFIG.isFilterListHasBiome(world.getBiome(new BlockPos((int) px2, 80, (int) pz2)))
									? getCloudDensityThreshold(densityByWeather, CONFIG.getDownfall(world.getBiome(new BlockPos((int) px2, 80, (int) pz2)).value().getPrecipitationAt(new BlockPos((int) px2, 80, (int) pz2))))
							...
							f = !CONFIG.isFilterListHasBiome(world.getBiome(new BlockPos((int) px, 80, (int) pz)))
									? getCloudDensityThreshold(densityByWeather, CONFIG.getDownfall(world.getBiome(new BlockPos((int) px, 80, (int) pz)).value().getPrecipitationAt(new BlockPos((int) px, 80, (int) pz))))
									: getCloudDensityThreshold(densityByWeather, densityByBiome);
						}
					} else {
					...
					if (CONFIG.isEnableTerrainDodge()) {
						for (int cy = 0; cy < height; cy++) {
							...
							_cloudData[cx][cy][cz] = world.getBrightness(LightLayer.SKY, new BlockPos(
									(int) px,
